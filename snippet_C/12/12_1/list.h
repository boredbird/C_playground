...
struct node * CreateForward();       //前插法新建链表为 12.1.2节方法②对应的函数
...
struct node * SearchNode(struct node * head,Type data,int condition) //单链表的查找
{ 
    ...
    while(p&&!equal(p->data,data,condition)) // equql函数用来判两个参数是否相等
