...
bool LoopCommand(LinkList &linkList)
{
    ...
    printf("6-查询元素位置  7-列举前驱  8-按内容删除元素  9-列举后继  0-退出\n");
    printf("\n请输入你要执行的操作代号：");
    fflush(stdin);
    command = getchar();
    fflush(stdin);
    switch (command) {
        ...
        case '8':
            printf("\n请输入要删除元素的值：");
            scanf("%d", &e);
            if (DeleteElem(linkList, e, equal) != TRUE) {
                printf("表中无此元素！\n");
            } else {
                printf("元素%d已删除！\n", e);
            }
