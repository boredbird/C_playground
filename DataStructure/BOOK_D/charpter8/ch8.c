...
#define MAXSIZE 3
typedef int keyType;
/*编写一个程序实现学生成绩管理，每个学生包括3门课的成绩，从键盘输入学生信息；
2.学生信息包括学号、姓名、三门课成绩，计算出学生的平均成绩，按照学生平均成绩由大到小排序。
要求分别采用直接插入排序、冒泡排序、快速排序、简单选择排序、堆排序、两路归并排序中的四种排序方件完成。
*/
typedef struct
{
	int stuNo;
...
void InputStuMsg(Student stu[],int n)
{
	...
	{
		printf("请输入第%d个学生的信息:\n",i);
		printf("学号：");scanf("%d",&stu[i].stuNo);
		printf("姓名：");scanf("%s",stu[i].name);
		printf("语文：");scanf("%f",&stu[i].score1);
		printf("数学：");scanf("%f",&stu[i].score2);
		printf("外语：");scanf("%f",&stu[i].score3);
...
/*将L.r[i]插入有序增量子表*/
while(j>=0&&r[0].avg<r[j].avg) /*将r[i]插入有序子表*/
...
/*对表r中的第low到第high个记录进行一次快速排序的划分，把关键字小于r[low].avg的记录放在r[low]前面，大于r[low].avg的记录放在r[low]后面*/
{  
	r[0]= r[low]; /* 把r[low]放在r[0]*/
     while (low<high) /*用r[low]进行一趟划分*/
{ 
   /*在high端,寻找一个比r[low].avg小的记录放入low*/
        while (low<high &&r[high].avg>=r[0].avg) --high;
        r[low]=r[high]; 
        /*在low端,寻找一个比r[low].avg大的记录放入high*/
...
{ 
		if (j<n&&r[j].avg>r[j+1].avg)j++;/*选取孩子中值较小的分支*/
